...
class GANModel(torch.nn.Module):
    def __init__(self, opt, netD_image):
        super().__init__()
        self.opt = opt
        ...
        self.netG, self.netD = self.initialize_networks(opt, netD_image)

        # netD_image is only created if image regularization is applied
        if isinstance(self.netD, list):
            self.netD_sketch, self.netD_image_old = self.netD
        else:
    ...
    # 修改data包含real_sketch, real_image, fake_image
    def forward(self, real_sketch, real_image, fake_image, mode):
        # real_sketch, real_image = self.preprocess_input(data)

        if mode == 'generator':
            g_loss, generated = self.compute_generator_loss(fake_image)
            return g_loss, generated
        elif mode == 'discriminator':
            d_loss, interm_imgs = self.compute_discriminator_loss(real_sketch, real_image, fake_image)
    ...
    # 在此修改，分别获取生成器和判别器的参数，需传入生成网络和判别网络
    def create_optimizers(self, opt):
        ...
        elif opt.optim_param_g == 'all_G':# 更新整个生成器
            G_param_names, G_params = get_param_by_name(self.netG, 'all_G')
        else:
    ...
    def initialize_networks(self, opt):
        ...
        netD_sketch = networks.define_D(opt)
    ...
    def compute_generator_loss(self, fake_image):
        G_losses = {}
    ...
    def compute_discriminator_loss(self, real_sketch, real_image, fake_image):
        D_losses = {}
        # 生成假图片,将generate_fake修改成psp_ref的生成器
        # 考虑当前代码结构，采取直接以参数形式的方式传入
        # with torch.no_grad():
        #     fake_image = self.generate_fake()
        #     fake_image = fake_image.detach()

        # Transform G's output to sketches to apply sketch loss
        # 将生成图像转换成sketch并送入判别器获得判别结果
        fake_transf = self.tf_fake(fake_image)
