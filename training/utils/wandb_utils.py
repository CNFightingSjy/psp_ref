...
class WBLogger:
    ...
    @staticmethod
    def log_images_to_wandb(x, r, y, y_hat, id_logs, prefix, step, opts):
        im_data = []
        column_names = ["Source", "Reference", "Target", "Output"]
        if id_logs is not None:
            column_names.append("ID Diff Output to Target")
        for i in range(len(x)):
            cur_im_data = [
                wandb.Image(common.tensor2gray(x[i])),
                wandb.Image(common.tensor2im(r[i])),
